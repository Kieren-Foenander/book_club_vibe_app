...
export function CurrentlyReading({ currentBook, clubId, isAdmin }: CurrentlyReadingProps) {
  ...
  const handleSubmitRating = async () => {
    if (!currentBook || storylineRating === 0 || charactersRating === 0 || spiceRating === 0) {
  ...
  const getRatingText = (rating: number, type: 'storyline' | 'characters' | 'spice') => {
    ...
    if (type === 'spice') {
      const texts = ["Mild - Sweet romance", "Warm - Some steamy scenes", "Hot - Regular spicy content", "Very Hot - Frequent spicy scenes", "Fire - Maximum spice level"];
      return texts[rating - 1];
    }
  ...
  if (!currentBook) {
    return (
      <div className="bg-white rounded-lg p-8 shadow-sm border border-gray-200">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">📖 Currently Reading</h2>
  ...
  return (
    <>
      <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="md:w-2/3">
            ...
            {book.summary && (
              <p className="text-gray-700 mb-6 text-sm leading-relaxed">{book.summary}</p>
            )}
            ...
            <div className="mb-6">
              ...
              <p className="text-sm text-gray-600">
                ...
                {totalPages > 0 && ` (${Math.round((userProgress / totalPages) * 100)}%)`}
              </p>
            </div>

            {allProgress.length > 0 && (
              <div>
                <h4 className="font-medium text-gray-700 mb-3">Club Progress</h4>
                <div className="space-y-2">
                  {allProgress.map((progress: any, index: number) => (
                    <div key={index} className="flex justify-between items-center text-sm">
                      <span className="text-gray-600">{progress.userName}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-gray-500">Page {progress.currentPage}</span>
                        {progress.percentage > 0 && (
                          <span className="text-pink-600 font-medium">{progress.percentage}%</span>
                        )}
                      </div>
                    </div>
                  ))}
      ...
      {showProgressUpdate && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Update Progress</h3>
      ...
      {showRatingModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Rate "{book.title}"</h3>
            ...
            <div className="space-y-6">
              ...
              <div>
                  ...
                  {Array.from({ length: 5 }, (_, i) => (
                    <button
                      ...
                      className={`text-3xl transition-all duration-200 hover:scale-110 ${
                        i < storylineRating ? "text-yellow-500 drop-shadow-sm" : "text-gray-300 hover:text-yellow-400"
                      }`}
                    >
                      ⭐
                    </button>
                ...
                <div className="text-center">
                  <span className={`text-sm font-medium ${storylineRating > 0 ? "text-yellow-600" : "text-gray-500"}`}>
              ...
              <div>
                ...
                <div className="flex gap-1 mb-2">
                  {Array.from({ length: 5 }, (_, i) => (
                    <button
                      ...
                      className={`text-3xl transition-all duration-200 hover:scale-110 ${
                        i < charactersRating ? "text-red-500 drop-shadow-sm" : "text-gray-300 hover:text-red-400"
                      }`}
                    >
                      ❤️
                    </button>
                ...
                <div className="text-center">
                  <span className={`text-sm font-medium ${charactersRating > 0 ? "text-red-600" : "text-gray-500"}`}>
              ...
              <div>
                ...
                <div className="flex gap-1 mb-2">
                  {Array.from({ length: 5 }, (_, i) => (
                    <button
                      ...
                      className={`text-3xl transition-all duration-200 hover:scale-110 ${
                        i < spiceRating ? "text-red-500 drop-shadow-sm" : "text-gray-300 hover:text-red-400"
                      }`}
                    >
                      🌶️
                    </button>
                ...
                <div className="text-center">
                  <span className={`text-sm font-medium ${spiceRating > 0 ? "text-red-600" : "text-gray-500"}`}>
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button
                ...
                disabled={isSubmitting || storylineRating === 0 || charactersRating === 0 || spiceRating === 0}
