                ...
                <div key={book._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-green-600">👍 {book.approvalCount}</span>
  ...
  return (
    <div className="max-w-md mx-auto">
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">🗳️ Tinder for Books</h2>
        <p className="text-gray-600">
          {unvotedBooks.length} book{unvotedBooks.length !== 1 ? 's' : ''} waiting for your vote
        </p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
        ...
        <div className="p-6">
          ...
          <h3 className="text-xl font-bold text-gray-800 mb-2">{currentBook.title}</h3>
          <p className="text-gray-600 mb-2">by {currentBook.author}</p>
          <p className="text-sm text-gray-500 mb-4">Suggested by {currentBook.suggesterName}</p>
          
          <div className="flex items-center gap-2 mb-4">
            <span className="text-sm text-gray-500">Spice Level:</span>
            <div className="flex">
              {Array.from({ length: 5 }, (_, i) => (
                <span key={i} className={i < currentBook.spiceRating ? "text-red-500" : "text-gray-300"}>
                  🌶️
                </span>
              ))}
            </div>
          ...
          {currentBook.summary && (
            <p className="text-gray-700 text-sm leading-relaxed mb-6">{currentBook.summary}</p>
          )}
        </div>
      ...
      {showVetoReasons && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Why are you passing?</h3>
